/*
  Colors — LED patterns, gradients and animations.

  The 10 keys form a 2-row x 5-column grid:

      1  2  3  4  5      row 0
      6  7  8  9 10      row 1

  Because keys 1 and 6 share a column, gradients are computed from
  column/row position rather than key index — running 1->10 linearly
  would look wrong on the physical board.

  Each key is a "bank", like a synth patch bank. Pressing key N selects
  bank N and paints the board with that bank's first look. Pressing the
  SAME key again advances that bank to its next look; each bank remembers
  its own step. Every bank stays on a single hue or a narrow analogous
  range so nothing clashes.

  Key 1 collects the flat "full color" looks (solids, gradients, duotone,
  saturation ramps). Keys 2-10 are shape and pattern variety — spots,
  checkerboard, chaser, diamond, twinkle.
...
void loop() {
  ...
  for (int key = KEY_LO; key <= KEY_HI; ++key) {
    if (ksm::press(key)) {
      int bank = key - KEY_LO;
      if (bank == gActiveBank) {
        // Same key pressed again: advance this bank to its next look.
